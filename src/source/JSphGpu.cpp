...
void JSphGpu::ParticlesDataUp(unsigned n,const tfloat3 *boundnormal){
  cudaError_t cudaStatus = cudaMemcpy(Idpg    ,Idp    ,sizeof(unsigned)*n,cudaMemcpyHostToDevice);  
  //fprintf(stderr, "cudaError ldpg : %d\n", cudaStatus);
  //fprintf(stderr, "ldpg : %d\n", sizeof(unsigned)*n);
  
  cudaError_t cudaStatus2 = cudaMemcpy(Codeg   ,Code   ,sizeof(typecode)*n,cudaMemcpyHostToDevice);
  //fprintf(stderr, "cudaError2 Codeg: %d\n", cudaStatus2);
  //fprintf(stderr, "Codeg : %d\n", sizeof(typecode)*n);

  cudaError_t cudaStatus3 = cudaMemcpy(Dcellg  ,Dcell  ,sizeof(unsigned)*n,cudaMemcpyHostToDevice);
  //fprintf(stderr, "cudaError3 Dcellg: %d\n", cudaStatus3);
  //fprintf(stderr, "Dcellg : %d\n", sizeof(unsigned)*n);

  cudaError_t cudaStatus4 = cudaMemcpy(Posxyg  ,Posxy  ,sizeof(double2)*n ,cudaMemcpyHostToDevice);
  //fprintf(stderr, "cudaError4 Posxyg: %d\n", cudaStatus4);
  //fprintf(stderr, "Posxyg : %d\n", sizeof(double2)*n);

  cudaError_t cudaStatus5 = cudaMemcpy(Poszg   ,Posz   ,sizeof(double)*n  ,cudaMemcpyHostToDevice);
  //fprintf(stderr, "cudaError5 Poszg: %d\n", cudaStatus5);
  //fprintf(stderr, "Poszg : %d\n", sizeof(double)*n);

  cudaError_t cudaStatus6 = cudaMemcpy(Velrhopg,Velrhop,sizeof(float4)*n  ,cudaMemcpyHostToDevice);
  //fprintf(stderr, "cudaError Velrhopg: %d\n", cudaStatus6);
  //fprintf(stderr, "Velrhopg : %d\n", sizeof(float4)*n);
